...
const FieldAnalysis = () => {
  ...
  const t = TRANSLATIONS[language].field;
  ...
  const runRainSimulation = async (amount) => {
    ...
    try {
      ...
      toast.success(t.toasts.rainSuccess, { icon: '🌧️' });
    } catch (err) {
  ...
  const runIrrigation = async () => {
    ...
    try {
      ...
      setPlot(res.data.data);
      toast.success(t.toasts.irrigationSuccess, { icon: '💧' });
    } catch (err) {
  ...
  const runHeatSim = async () => {
    ...
    try {
      ...
      setPlot(res.data.data || res.data); 
      toast.success(t.toasts.heatSuccess, { icon: '☀️' });
      fetchStatus(true);
    } catch (err) {
  ...
  const runReset = async () => {
    const confirmMsg = language === 'ml' ? "ഉറപ്പാണോ? എല്ലാ സിമുലേഷൻ ഡാറ്റയും റീസെറ്റ് ചെയ്യപ്പെടും." : "Are you sure? This will reset all simulated soil data.";
  ...
  const getMoistureUI = (val) => {
    if (val < 25) return { bar: "bg-red-500", text: "text-red-600", label: t.stats.criticallyDry, icon: "text-red-400" };
    if (val < 50) return { bar: "bg-amber-500", text: "text-amber-600", label: t.stats.needsWater, icon: "text-amber-400" };
    return { bar: "bg-emerald-500", text: "text-emerald-600", label: t.stats.optimal, icon: "text-emerald-400" };
  };
        ...
        <div className="max-w-6xl mx-auto relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div>
            <div className="flex items-center gap-3 mb-4">
              <span className="bg-emerald-500 text-white px-3 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest">{t.hero.online}</span>
            ...
            <p className="text-slate-400 font-bold flex items-center gap-2">
              <MapPin size={16} className="text-emerald-500" /> {t.hero.sensor} ID: {plot._id.slice(-6).toUpperCase()} • {plot.cropHistory.currentCrop}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
             <button onClick={() => setShowRainModal(true)} className="bg-white/10 hover:bg-white/20 text-white px-6 py-4 rounded-2xl flex items-center gap-3 font-bold border border-white/10 backdrop-blur-md transition-all active:scale-95">
                <CloudRain size={20} className="text-blue-400" /> {t.hero.rain}
             </button>
             <button onClick={runIrrigation} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-4 rounded-2xl flex items-center gap-3 font-bold transition-all active:scale-95 shadow-xl shadow-emerald-500/20">
                <Droplet size={20} /> {t.hero.irrigate}
             </button>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 -mt-12 relative z-20 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{t.stats.moisture}</p>
          ...
          <StatCard label={t.stats.nitrogen} value={simulationState.estimatedNutrients.nitrogen} icon={<Activity />} color="text-emerald-500" bg="bg-emerald-50" />
          <StatCard label={t.stats.ph} value={simulationState.estimatedNutrients.pH} icon={<Thermometer />} color="text-amber-500" bg="bg-amber-50" />
          <StatCard label={t.stats.health} value={`${simulationState.healthScore}/100`} icon={<CheckCircle2 />} color="text-purple-500" bg="bg-purple-50" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-[3rem] p-8 shadow-xl shadow-slate-200 border border-slate-100">
                <h3 className="font-black text-slate-900 mb-6 flex items-center gap-2">
                    <Zap size={18} className="text-amber-500" /> {t.command.title}
                </h3>
                <div className="space-y-3">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-orange-900">{t.command.heatwave}</span>
                        </div>
                    ...
                    >
                        <div className="flex items-center gap-3 text-slate-600 group-hover:text-red-600">
                            <RotateCcw size={20} />
                            <span className="font-bold">{t.command.reset}</span>
                        </div>
                ...
                <div className="mt-8 pt-8 border-t border-slate-50">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{t.command.impact}</p>
                    <div className="flex justify-between items-center text-sm font-bold text-slate-700">
                        <span>{t.command.lastRain}</span>
                        <span className="text-blue-500">{weatherContext.lastRainfallMM}mm</span>
                    </div>
                </div>
            </div>
            
            <DosageCalculator translation={t.dosage} />
          ...
          <div className="lg:col-span-8 bg-white rounded-[3rem] p-10 shadow-2xl shadow-slate-200 border border-slate-100">
            <h2 className="text-2xl font-black text-slate-900 mb-8 flex items-center gap-3">
              <Waves className="text-emerald-600" /> {t.guidance.title}
            </h2>
            ...
            {recommendations.length > 0 ? (
              <div className="space-y-4">
                {recommendations.map((rec, i) => (
                  <div key={i} className="flex items-start gap-5 p-6 rounded-3xl bg-slate-50 border-l-4 border-emerald-500">
                    ...
                    <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">{rec.category} • {rec.priority} {t.guidance.priority}</p>
                      <p className="text-lg font-bold text-slate-800 leading-tight mb-2">{language === 'ml' ? rec.messageMl : rec.messageEn}</p>
                      {language === 'en' && <p className="text-sm font-medium text-slate-500 italic mb-4">{rec.messageMl}</p>}
                      <span className="text-[10px] font-black bg-emerald-100 text-emerald-700 px-3 py-1 rounded-lg uppercase">{t.guidance.action}: {rec.dosage}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="p-16 text-center border-2 border-dashed border-slate-100 rounded-[3rem]">
                ...
                <p className="font-black uppercase tracking-widest text-slate-300 text-xs">{t.guidance.optimalLimits}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 backdrop-blur-md bg-slate-900/40 animate-in fade-in duration-300">
            <div className="bg-white rounded-[3.5rem] p-10 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-300 relative border border-slate-100">
                ...
                <div className="mb-8 text-center">
                    ...
                    <h3 className="text-2xl font-black text-slate-900 tracking-tight leading-none">{t.rainModal.title}</h3>
                    <p className="text-[10px] font-black text-slate-400 mt-2 uppercase tracking-widest">{t.rainModal.subtitle}</p>
                </div>
                <div className="grid grid-cols-1 gap-3">
                    {[
                        { val: 5, label: t.rainModal.light, color: 'hover:bg-blue-50 hover:border-blue-200' },
                        { val: 20, label: t.rainModal.moderate, color: 'hover:bg-blue-100 hover:border-blue-300' },
                        { val: 50, label: t.rainModal.heavy, color: 'hover:bg-blue-600 hover:text-white' }
                    ].map((btn) => (
                        <button key={btn.val} onClick={() => runRainSimulation(btn.val)} className={`w-full p-6 border-2 border-slate-100 rounded-[2rem] text-left transition-all group flex justify-between items-center ${btn.color}`}>
                            <div><p className="font-black">{btn.label}</p><p className="text-[10px] opacity-60 uppercase font-bold">{btn.val}mm {t.rainModal.volume}</p></div>
...
const SetupModal = ({ onCreated }) => {
  const { language } = useLanguage();
  const t = TRANSLATIONS[language].field.setup;
  ...
  const handleSetup = async (e) => {
    ...
    try {
      ...
      onCreated(res.data);
    } catch (err) { toast.error(TRANSLATIONS[language].field.toasts.setupFail); }
      ...
      <div className="bg-white rounded-[3.5rem] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="bg-[#0F172A] p-10 text-white relative">
          <div className="relative z-10 flex items-center gap-6">
            ...
            <div>
              <h2 className="text-3xl font-black tracking-tight leading-none">{t.title}</h2>
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-2">{t.subtitle}</p>
            </div>
          </div>
        </div>
        <form onSubmit={handleSetup} className="p-10 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputGroup label={t.plotName} placeholder={language === 'ml' ? "ഉദാ: തെക്കേ പറമ്പ്" : "e.g. South Field"} onChange={v => setFormData({...formData, plotInfo: {...formData.plotInfo, name: v}})} />
            <InputGroup label={t.cropType} placeholder={language === 'ml' ? "ഉദാ: വാഴ" : "e.g. Banana"} onChange={v => setFormData({...formData, cropHistory: {...formData.cropHistory, currentCrop: v}})} />
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-4 tracking-widest">{t.soilType}</label>
              <select className="w-full bg-slate-50 border-none rounded-3xl p-5 font-bold appearance-none outline-none" onChange={e => setFormData({...formData, plotInfo: {...formData.plotInfo, soilType: e.target.value}})}>
                <option value="laterite">{t.soilOptions.laterite}</option>
                <option value="black_soil">{t.soilOptions.black}</option>
                <option value="alluvial">{t.soilOptions.alluvial}</option>
              </select>
            </div>
          </div>
...
const DosageCalculator = ({ translation }) => {
  const [size, setSize] = useState(1);
  const [unit, setUnit] = useState('acre');
  const [chemical, setChemical] = useState('neem_oil');

  const chemicals = {
    npk_19: { name: translation.chemicals.npk_19, calc: (acres) => `${(5 * acres).toFixed(1)} Kg` },
    urea: { name: translation.chemicals.urea, calc: (acres) => `${(45 * acres).toFixed(1)} Kg` },
    neem_oil: { name: translation.chemicals.neem_oil, calc: (acres) => `${(1 * acres).toFixed(1)} Liters + ${(200 * acres).toFixed(0)}L Water` },
    copper_50wp: { name: translation.chemicals.copper_50wp, calc: (acres) => `${(1 * acres).toFixed(1)} Kg` },
    bordeaux: { name: translation.chemicals.bordeaux, calc: (acres) => `${(4 * acres).toFixed(1)}kg CuSO4 + ${(4 * acres).toFixed(1)}kg Lime` },
    tricho: { name: translation.chemicals.tricho, calc: (acres) => `${(2.5 * acres).toFixed(1)} Kg + ${(50 * acres).toFixed(0)}kg Compost` },
  };
  ...
  return (
    <div className="bg-white rounded-[3rem] p-8 shadow-sm hover:shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden group hover:border-indigo-200 transition-all">
      ...
      <h3 className="font-black text-slate-900 mb-6 flex items-center gap-2 relative z-10">
        <FlaskConical size={18} className="text-indigo-500" /> {translation.title}
      </h3>
      ...
      <div className="space-y-4 relative z-10">
        <div className="flex gap-3">
            ...
            >
              <option value="acre">{translation.unitAcre}</option>
              <option value="hectare">{translation.unitHectare}</option>
              <option value="cent">{translation.unitCent}</option>
            </select>
        </div>
        ...
        <div className="mt-6 pt-2">
            <div className="p-5 bg-indigo-50 rounded-2xl border border-indigo-100 flex flex-col items-center justify-center text-center group-hover:border-indigo-300 transition-colors">
                <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1.5">{translation.outputLabel}</p>
